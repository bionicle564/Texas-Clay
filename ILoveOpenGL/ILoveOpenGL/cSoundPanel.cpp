...
{
	std::stringstream musicListStream;
	musicListStream << "assets\\audio\\musiclist.txt";

	//Read the text file one line at a time and save the line into my vector
	std::ifstream inputFile(musicListStream.str());
	...
	if (inputFile.is_open())
	{
		...
		{
			std::stringstream musicStringStream;
			musicStringStream << "assets\\audio\\music\\" << musicName;

			//Creates a stream 
			FMOD::Sound* music;
			result = system->createStream(musicStringStream.str().c_str(), FMOD_LOOP_NORMAL, 0, &music);
			if (result != FMOD_OK)
			{
...
{
	std::stringstream soundListStream;
	soundListStream << "assets\\audio\\soundeffectslist.txt";

	//Read the text file one line at a time and save the line into my vector
	std::ifstream inputFile(soundListStream.str());
	...
	if (inputFile.is_open())
	{
		...
		{
			std::stringstream soundStringStream;
			soundStringStream << "assets\\audio\\soundeffects\\" << soundName;

			//Creates a sample 
			FMOD::Sound* soundEffect;
			result = system->createSound(soundStringStream.str().c_str(), FMOD_CREATESAMPLE, 0, &soundEffect);
			if (result != FMOD_OK)
			{
...
{
	std::stringstream bgListStream;
	bgListStream << "assets\\audio\\bglist.txt";

	//Read the text file one line at a time and save the line into my vector
	std::ifstream inputFile(bgListStream.str());
	...
	if (inputFile.is_open())
	{
		...
		std::stringstream bgStringStream;
		bgStringStream << "assets\\audio\\bg\\" << bgName;

		//Creates a sample 
		FMOD::Sound* backGroundStream;
		result = system->createStream(bgStringStream.str().c_str(), FMOD_LOOP_NORMAL, 0, &backGroundStream);
		if (result != FMOD_OK)
		{
